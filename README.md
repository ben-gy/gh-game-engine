...
| `lobby.ts` | Drop-in lobby **view** over `rematch.ts`: room code, invite link + Web Share, **join QR**, player roster, ready states, host-only Start, connecting spinner, the *"Host this room"* offer, the unseated *"round in progress"* state, and the `?netdebug=1` overlay. | Every multiplayer game. |
| `qr.ts` | **Join QR** — `toSvg(link)` / `encodeQr(link)`. Self-contained encoder (byte mode, versions 1–10, all four ECC levels), inline SVG, **zero dependencies and no image service**. Already wired into `lobby.ts`; import directly only to place a QR somewhere else. | Every multiplayer game (free via the lobby). |
